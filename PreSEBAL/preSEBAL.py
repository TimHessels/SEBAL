...
####################################################################################################################
    ...
    inputExcel= r"G:\SEBAL_Java\InputEXCEL_v3_3_6_JAVA_PROBAV_VIIRS_s5.xlsx"                    #r'G:\SEBAL_Tadla\Excel\InputEXCEL_v3_3_6.xlsx'               # The excel with all the SEBAL input data
    ...
    output_folder =  r'G:\SEBAL_Java\Preprocessing_Output'                #r'G:\SEBAL_Tadla\Preprocessing_Output'         # Output folder
    ...
    for number in range(2,endExcel+1):
        ...
        SEBAL_RUNS[number] = {'input_folder': input_folder, 'image_type': Image_Type}
    ...
    ######################## Extract general data for VIIRS-PROBAV ##########################################	
        if Image_Type == 2:	
            ...
            # else use PROBA-V day but than no transmissivity can be determined for now
                ...
                Date_PROBAV = str(Meta_data['LEVEL3_RADIOMETRY_BLUE_OBSERVATION_END_DATE'])
        ...
        if Determine_transmissivity == 1:
            ...
            if Value_Method_Radiation_24 == 2:
                ...
                if Field_Radiation_24 == 'None':
                    ...
                    # Reproject the Ra_inst data to match the LANDSAF data
                    Ra_24_3Km_dest, ulx, lry, lrx, uly, epsg_to = SEBAL.reproject_dataset_example(Ra_mountain_24_fileName, file_Landsaf_inst, method = 1)   
                    ...
                    # Reproject Transmissivity to match DEM (now this is done by using the nearest neighbour method)
                    Transmissivity_24_dest, ulx, lry, lrx, uly, epsg_to = SEBAL.reproject_dataset_example(Transmissivity_24_3Km_fileName, cos_zn_fileName, method = 3)  
    ...
    ################### Extract Meteo data for Landsat days from SEBAL Excel ##################
     ...
     #################### Calculate NDVI and SAVI for VIIRS-PROBAV ##########################################	
        ...
        if Image_Type == 2:	
            ...
            if Name_PROBAV_Image is not 'None':
...
# Data parameters
rasters_path = r'G:\SEBAL_Java\VIIRS_PROBAV_beforeHANTS\VIIRS_TIFF_in_WGS84'
name_format = 'GITCO_npp_d{0}_noaa_ops.tif'
start_date = '2016-01-01'
end_date = '2016-12-31'
latlim = [-6.83, -6.15]
lonlim = [107.75, 108.58]
cellsize = 0.00398898
nc_path = r'G:\SEBAL_Java\LST_VIIRS4.nc'
rasters_path_out = r'G:\SEBAL_Java\HANTS_VIIRS_OUT_RUN4_365_100_285_350_2_01_1'

# HANTS parameters for LST
nb = 365 # nr of images
nf = 100   # number of frequencies to be considered above the zero frequency
low = 285 # valid range minimum
high = 350 # valid range maximum
HiLo = 'Lo' # 2-character string indicating rejection of high or low outliers
fet = 2  # fit error tolerance (point eviating more than fet from curve fit are rejected)
