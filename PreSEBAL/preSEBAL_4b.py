...
####################################################################################################################
    ...
    for i in np.append(Kind_Of_Runs_Dict[2],Kind_Of_Runs_Dict[3]):
        ...
        Rs_in_filename_inst = '%s' %str(ws3['M%d' %i].value)
        ...
        if os.path.exists(LST_filename):
            ...
            # Cold pixels water
            ts_dem_cold,cold_pixels,ts_dem_cold_mean = Calc_Cold_Pixels(ts_dem, WM, QC_Map, ts_dem_cold_veg, Cold_Pixel_Constant, DEM)
...
def Calc_Cold_Pixels(ts_dem,water_mask,QC_Map,ts_dem_cold_veg,Cold_Pixel_Constant, DEM):
    """
    ...
    cold_pixels[DEM<=0]=np.nan
    cold_pixels[np.isnan(DEM)]=np.nan
    ...
    ts_dem_cold = np.nanpercentile(cold_pixels, 50)
