...
for filename in re[:]:
    ...
    Date = datetime.strptime('_'.join(filename.split("_")[2:4])[:-3], "d%Y%m%d_t%H%M")
    
    outfile = os.path.join(output_folder, "NPP_VIAES_L1_%d%02d%02d_H%02dM%02d.tif" %(Date.year, Date.month, Date.day, Date.hour, Date.minute))
