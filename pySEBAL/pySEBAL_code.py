...
def main(number, inputExcel):
    ...
    import SEBAL.pySEBAL.pySEBAL_input_USER as input_USER
    ...
    if (Image_Type == 2 or Image_Type == 3 or Image_Type == 4):
    ...
    try:
        ...
        print('Average QC temp = %s\n' %float(np.nanmean(QC_temp)))
    except:
       QC_temp = None 
    ...
    if Image_Type == 4:
        Surf_albedo, NDVI, LAI, vegt_cover, FPAR, Nitrogen, tir_emis, water_mask_temp, QC_Map = input_USER.Get_USER_Para_Veg(wb, number, proyDEM_fileName, year, month, day)
    ...
    if Image_Type == 4:
        Surface_temp, cloud_mask_temp, Thermal_Sharpening_not_needed = input_USER.Get_USER_Para_Thermal(wb, number, proyDEM_fileName, year, month, day, water_mask_temp, surf_temp_offset, Thermal_Sharpening_not_needed)
    ...
    Rn_24,rn_inst,g_inst,Rnl_24_FAO, Rn_24_FAO, Rn_24_Slob = Calc_Meteo(Rs_24,eact_24,Temp_24,Surf_albedo,dr,tir_emis,temp_surface_sharpened,water_mask,NDVI,Transm_24,SB_const,lw_in_inst,Rs_inst)
...
def Calc_ETact(esat_24, eact_24, EF_inst, Rn_24, Refl_rad_water, Lhv, Image_Type):
    ...
    """
    # Advection factor
    if (Image_Type == 2 or Image_Type == 4):
...
#------------------------------------------------------------------------------
def Correct_Surface_Temp_Lapse_Rate(ts_corr, DEM_resh, NDVI, slope, water_mask, ClipLandsat, QC_temp = None):
    """
    Function to correct the surface temperature based on the DEM map
    """
    ...
    if len(ts_corr_flatten) > 100:
        ...
        if len(y_values) > 0:
            ...
            Temp_array_lapse_rate = np.where(QC_temp == 1, Temp_array_lapse_rate, (DEM_resh - DEMmin) * -0.0065)
            ts_dem = ts_corr - Temp_array_lapse_rate
            
        else:
            ts_dem = ts_corr + 0.0065 * (DEM_resh - DEMmin)
            
    else:
        DEMmin = np.nanmin(np.nanpercentile(DEM_resh, 0.05))
        ts_dem = ts_corr + 0.0065 * (DEM_resh - DEMmin)
...
def Calc_Meteo(Rs_24,eact_24,Temp_24,Surf_albedo,dr,tir_emis,Surface_temp,water_mask,NDVI,Transm_24,SB_const,lw_in_inst,Rs_inst):
    ...
    return(Rn_24,rn_inst,g_inst,Rnl_24_FAO, Rn_24_FAO, Rn_24_Slob)
...
def Reshape_Reproject_Input_data(input_File_Name, output_File_Name, Example_extend_fileName, resampling_method = 3):
   ...
   data_rep, ulx_dem, lry_dem, lrx_dem, uly_dem, epsg_to = reproject_dataset_example(
       dest_input, Example_extend_fileName, method = resampling_method)
...
def Thermal_Sharpening(surface_temp_up, NDVI_up, NDVI, Box, dest_up, output_folder, ndvi_fileName, dest_down, watermask = False):
    ...
    CoefA_Downscale = Save_as_MEM(CoefA, geo, proj)
    CoefB_Downscale = Save_as_MEM(CoefB, geo, proj)
    CoefC_Downscale = Save_as_MEM(CoefC, geo, proj)

    # Downscale the fitted coefficients
    CoefA_Downscaled, ulx_dem, lry_dem, lrx_dem, uly_dem, epsg_to = reproject_dataset_example(
                                  CoefA_Downscale, dest_down)    
    CoefB_Downscaled, ulx_dem, lry_dem, lrx_dem, uly_dem, epsg_to = reproject_dataset_example(
                                  CoefB_Downscale, dest_down)    
    CoefC_Downscaled, ulx_dem, lry_dem, lrx_dem, uly_dem, epsg_to = reproject_dataset_example(
                                  CoefC_Downscale, dest_down)        
